...
def extract_phase_overview_context(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    structured_plan = state.get("structured_training_plan") or structured_report.get("structured_training_plan") or {}
    if not isinstance(structured_plan, dict):
        structured_plan = {}

    phase_summary = structured_plan.get("phase_summary") or structured_report.get("phase_summary") or []
    ...
    week_sections = structured_report.get("training_plan_weeks") or structured_plan.get("week_plans") or []
    if not isinstance(week_sections, list):
        week_sections = []
    ...
    current_week = next(
        (
            week
            for week in week_sections
            if isinstance(week, dict) and _safe_int(week.get("week_index"), 0) == 1
        ),
        None,
    ) or next((week for week in week_sections if isinstance(week, dict)), None)
    if isinstance(current_week, dict):
        current_week_index = _safe_int(current_week.get("week_index"), 1) or 1

    phases = []
    for item in phase_summary:
        if not isinstance(item, dict):
            continue
        start_week = _safe_int(item.get("start_week"), 0)
        end_week = _safe_int(item.get("end_week"), 0)
    ...
    actual_weeks = _safe_int(plan_meta.get("actual_weeks"), 0) or _safe_int(plan_meta.get("requested_weeks"), 0)
    if not actual_weeks and week_sections:
        actual_weeks = len([week for week in week_sections if isinstance(week, dict)])

    current_phase = next(
        (
            phase
            for phase in phases
            if _safe_int(phase.get("start_week"), 0) <= current_week_index <= _safe_int(phase.get("end_week"), 0)
        ),
        None,
    )
    current_phase_name = _safe_text(
        current_week.get("phase") if isinstance(current_week, dict) else "",
        _safe_text(current_phase.get("phase") if isinstance(current_phase, dict) else "", _safe_text(overview.get("current_phase"), "")),
    )

    return {
        "actual_weeks": actual_weeks,
        "requested_weeks": _safe_int(plan_meta.get("requested_weeks"), actual_weeks),
        "phases": phases,
        "phase_count": len(phases),
        "goal": _safe_text(plan_meta.get("goal"), _safe_text(overview.get("goal"), "训练计划")),
...
def build_phase_overview_bar_props(context: Dict[str, Any]) -> Dict[str, Any]:
    """将阶段总览上下文转换为 PhaseOverviewBar CustomElement 的 props"""
    if not context:
        return {}

    phases = []
    for phase in context.get("phases") or []:
        if not isinstance(phase, dict):
            continue
        phases.append({
            "phase": _safe_text(phase.get("phase"), "训练期"),
            "start_week": _safe_int(phase.get("start_week"), 1),
            "end_week": _safe_int(phase.get("end_week"), 1),
            "week_count": _safe_int(phase.get("week_count"), 1),
            "objective": _safe_text(phase.get("objective"), ""),
        })
    ...
    actual_weeks = _safe_int(context.get("actual_weeks"), 0) or max(
        _safe_int(phase.get("end_week"), 0)
        for phase in phases
    )
    current_week = _safe_int(context.get("current_week"), 1) or 1

    return {
        "plan_meta": {
            "goal": _safe_text(context.get("goal"), "训练计划"),
            "actual_weeks": actual_weeks,
            "requested_weeks": _safe_int(context.get("requested_weeks"), actual_weeks),
        },
        "phase_summary": phases,
        "current_week": current_week,
        "current_phase": _safe_text(context.get("current_phase"), ""),
        "interaction_mode": "vertical_scroll",
    }
...
def extract_current_week_context(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    default_expanded_day_indexes: List[int] = []
    first_workout_day_index = 0
    for day_idx, day in enumerate((current_week.get("days") or []) or [], start=1):
        ...
        if not first_workout_day_index and _safe_text(day.get("training_type"), "未安排") != "休息":
            first_workout_day_index = day_idx
    ...
    return {
        ...
        "days": days_data,
        "default_expanded_day_indexes": [first_workout_day_index] if first_workout_day_index else default_expanded_day_indexes,
...
def build_week_training_card_props(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "default_expanded_day_indexes": context.get("default_expanded_day_indexes") or [],
        "key_workouts": context.get("key_workouts") or [],
        "action_suggestions": context.get("action_suggestions") or [],
