...
- 沿 `research_analyst -> formatter -> structured_report.summary -> report_ui.render_structured_report` 链路排查“交叉研究回答只显示一半”的问题，确认不是模型停止生成，而是格式化阶段把正文主展示字段截断了。
...
- 修复范围限定在格式化层，降低对 `旧前端`、`旧前端` 和其他节点链路的回归影响。

## 补充记录：源头修复引用按钮 PDF 路径

### 1. 做了什么

- 继续沿 `检索命中 -> rag_sources -> structured_report.evidence_base -> 旧前端 view_pdf` 链路排查，确认“按钮有响应但提示找不到文件”的根因不是回调，而是知识库索引产物只保存了文件名，没有保存文档绝对路径。
...
- `旧前端` 引用按钮生成时能够拿到真实 PDF 绝对路径，点击后可直接进入 PDF 预览链路。
...
- 在 `marathon_qa_assistant/ui/report_ui.py` 中移除正文与“参考来源”区块的 `action:view_pdf?...` 链接生成，改为只保留纯文本引用标记 `[n]`。
...
- 旧前端 预览动作链路：`build_evidence_preview_bundle()`、`旧前端动作(name="view_pdf", ...)`、`PDF 预览组件(display="side")`。
- 组间实现方法：渲染组只负责稳定展示引用编号，交互组统一负责按钮动作注册，避免让 Markdown 链接同时承担展示和交互两种职责。

### 3. 实现了什么效果

- 报告中的引用编号不再被前端渲染成会新开页面的普通 `<a>` 链接。
- PDF 预览入口收敛为 旧前端 显式按钮，点击后行为固定为当前界面侧边栏预览。
- 引用展示层与预览动作层职责分离，后续维护时不容易再次出现“文字可点但动作链路失真”的问题。

## 补充记录：删除 旧前端 多模态实验室按钮

### 1. 做了什么

- 检查 `marathon_qa_assistant/apps/旧前端入口文件` 的欢迎区 Action 配置，确认 `Coach Mode` 与 `Research Mode` 两套首页按钮都显式包含 `multimodal_lab` 入口。
- 仅从 `旧前端` 首页欢迎区移除“多模态实验室”按钮，不改动多模态回调、知识图谱注入能力和其他入口代码，保持最小影响面。
- 将本次改动记录到实验文档，明确这是一次前端入口收敛，而不是后端多模态能力下线。

### 2. 用了什么

- 旧前端 `旧前端动作` 欢迎区按钮列表。
- `marathon_qa_assistant/apps/旧前端入口文件` 单文件首页渲染逻辑。
- 组间实现方法：仅调整 UI 入口层，保留 `multimodal_lab` / `inject_multimodal` 回调与 `services/multimodal.py`，避免影响图谱构建组和多模态能力组的后端接口兼容。

### 3. 实现了什么效果

- `旧前端` 首页不再显示“多模态实验室”按钮，界面入口更聚焦于训练画像、计划调整、图谱搜索和知识库管理。
- 已存在的多模态后端逻辑仍保留，后续若需要恢复入口，只需重新挂回对应 `Action`，无需重写服务层。
- 改动范围限定在 `旧前端` 欢迎区，不影响 `旧前端`、`FastAPI` 及其余业务流程。
...
- 旧前端 `前端会话态` 会话态，用于保存当前训练画像步骤的字段和消息 ID。
- 旧前端 Action 回调中的 `forId` / `for_id` 消息定位信息。
- 原消息 `消息更新` 刷新机制，与当前步骤弱绑定校验组合使用。
...
- 在单选、多选、确认、自定义等训练画像回调中，优先使用 Action 自带消息 ID；若缺失则回退到会话中记录的当前步骤消息 ID 做 `消息更新`。

### 2. 用了什么

- 旧前端 `消息发送` 返回的消息对象及其 `id`。
- `前端会话态` 会话态，用于持久化当前训练画像步骤的消息定位信息。
- `消息更新` 原地刷新按钮状态，避免重复发送同一步骤消息。
...
- 检查 `marathon_qa_assistant/apps/旧前端入口文件` 中训练画像向导的 `pick_option`、`toggle_option`、`confirm_multi`、`profile_custom` 四条交互链路。
...
- 旧前端 Action 回调机制与 `消息更新` 原地更新消息能力。
- 旧前端 普通消息入口 `main()` 与会话态 `user_session` 字段暂存机制。
- 本地源码定位方式：直接核对 旧前端 `Action` 数据结构的实际属性名。
- 代码验证方式：对 `旧前端入口文件` 执行 Diagnostics 检查，并使用 `python -m py_compile` 做语法编译验证；再通过浏览器逐步实测 `填写训练画像 -> 精英 -> 自定义输入 69 -> 无近期比赛`。
...
- **过程**:
  ...
  - 将原 `integrated_platform.py` 精简为纯入口文件（`app.py` 角色），负责组合各模块的 旧前端 Tab 并绑定跨模块事件（如知识库图谱构建后同步更新研究区的图谱）。
...
- **过程**:
  ...
  - **UI 适配**: 在 `旧前端入口文件` 中增加对 `wiki_search` 节点的实时追踪，并在最终报告中新增 `🌐 维基百科知识增强` 区块。
...
- **过程**:
  ...
  - **UI 交互集成**: 在 [旧前端入口文件](file:///c:/Users/26318/Documents/trae_projects/ollama_pro/exp5_graphrag/旧前端入口文件) 的 `Research Mode` 中新增“多模态实验室”功能，支持用户上传跑步科学图表并获取多模型分析结果。
...
### 1. 做了什么
- 针对“PDF 预览按钮偶尔提示找不到文件”的问题，在 `旧前端入口文件` 和 `已移除的旧前端动作文件` 的 `view_pdf` 回调中增加了防御性路径补全逻辑。
...
- 在 `已移除的旧前端配置文件` 中更新了 Z1-Z9 的专业中文标签，同步侧边栏显示。
