...
\begin{enumerate}
  ...
  \item \textbf{Benchmark artifact}: M-EXRxBench provides a 500-case balanced synthetic benchmark with system-visible/evaluator-only splits, eligibility checks, forbidden-output labels, and a supplemental 100-case stress set.
...
Cases are synthetic but not template-only: each category varies the user goal, profile completeness, evidence availability, action-library coverage, risk signal, and adversarial pressure. The labeled split records expected status, risk level, required rules, forbidden outputs, and rationale, while the system-visible split removes these fields. Figure~\ref{fig:benchmark-protocol} summarizes this split and shows that the stress-test set is reported separately rather than merged into the default 500-case score.
...
M-EXRxBench is synthetic and contains no real patient records, athlete telemetry, wearable streams, or private health information. The cases test rule boundaries, not real-world prevalence. A real deployment would require prospective validation, domain-expert governance, localized guidance, distribution-shift monitoring, privacy review, and clear escalation pathways for red-flag symptoms.
