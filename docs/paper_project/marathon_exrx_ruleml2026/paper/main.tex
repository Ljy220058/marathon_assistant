...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=\linewidth]{../figures/rule_governed_workflow.pdf}
...
The coarse baselines above isolate broad failure modes. To make the contribution of each governance component explicit, we run five knock-out ablations over the same 500-case benchmark, each removing one component from the full rule-governed solver while leaving the evaluator and output schema unchanged. These are deterministic ablation controls, not externally run LLM systems. Table~\ref{tab:fine-ablation} reports the resulting loss-of-function signatures. Trace completeness remains 1.000 for these runs because all outputs are schema-normalized before evaluation; the informative changes are in risk accuracy, unsafe advice, unsupported prescription, status accuracy, and repair success.

\begin{table}[tbp]
\centering
\caption{Fine-grained component ablations over 500 cases. Each row removes one component from the full rule-governed solver; contribution is inferred from the resulting metric degradation under the same deterministic benchmark and evaluator.}
...
The ablations give distinct failure signatures, so the reported improvement is not attributable to a single monolithic rule wrapper. RiskGate contributes risk stratification and high-risk refusal: removing it drops risk accuracy from 1.000 to 0.240 and reintroduces unsafe advice. EvidenceGate contributes prescription authority checks: removing it raises unsupported prescriptions to 0.684. The PrescriptionContract contributes status-boundary enforcement: removing it reduces status accuracy to 0.440. Bounded repair contributes the audited repair pathway: removing it leaves status and risk labels unchanged in this deterministic setup, but repair success falls from 1.000 to 0.000. The Rule Auditor contributes the final veto over unsafe candidates: removing it reintroduces unsafe leakage and lowers repair success. These results establish component necessity within this artifact and benchmark; they do not by themselves establish clinical safety or real-world deployment readiness.
