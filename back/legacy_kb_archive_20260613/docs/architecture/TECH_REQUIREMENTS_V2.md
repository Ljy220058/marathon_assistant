...
- **防御性路径补全 (Defensive Path Fallback)**: 在 `view_pdf` 动作回调（`chainlit_app.py` 及 `actions.py`）中增加兜底逻辑：若 `payload.path` 缺失、失效或仅为纯文件名，系统将自动调用 `infer_source_path()` 在 `data/uploads/seed`、`domain_docs` 等预设目录中搜索匹配的绝对路径，确保预览的高可用性。
...
- 本阶段不修改 `apps/web/` CustomElement 组件，不新增独立 HMP 前端卡片。
...
- `tools/kb/evaluate_rag_ragas.py`：
...
- 健康检查只报告 RAG 运行时可用性，不代表某个具体问题一定能召回正确证据；召回质量仍由 `tools/kb/evaluate_rag_ragas.py` 的评测集负责。
...
- 组件位置：`apps/web/src/pages/index.astro` 的 Plan Builder 区域，紧跟生成/取消工具栏。
...
- 默认端口：`apps/web/src/pages/index.astro` 的 `API Base` 默认值调整为 `http://127.0.0.1:8010`，与当前推荐的 FastAPI 启动端口一致。
