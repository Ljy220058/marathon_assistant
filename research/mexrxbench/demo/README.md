...
- Static reviewer trace inspector: `demo/static_trace_viewer.html`.
...
The inspector includes three embedded examples for quick reviewer inspection:
`mexrx-011` shows R2 bounded repair, `mexrx-024` shows R3 fail-closed refusal,
and `mexrx-002` shows R0 explain-only behavior. It separates system-visible
case data from evaluator-only labels and reports trace completeness as
required-field presence, not semantic proof.

The demo is a rule-compliance artifact over a synthetic benchmark. It does not call an external LLM and does not provide clinical validation, deployment safety evidence, generalization evidence, or athlete-outcome proof.
