...
RUN_ARTIFACT_ROOT = REPO_ROOT / "artifacts" / "research_runs" / "mexrxbench"
...
def artifact_label(path: Path, root: Path) -> str:
    if root == RUN_ARTIFACT_ROOT:
        return "artifacts/" + path.relative_to(root).as_posix()
    return rel(path)
...
def resolve_manifest_path(relative: str) -> Path:
    ...
    if len(parts) >= 2 and parts[0] == "artifacts" and parts[1] in {"demo_runs", "baseline_runs"}:
        migrated = RUN_ARTIFACT_ROOT / Path(*parts[1:])
        if migrated.exists():
            return migrated
    return path
...
def validate_outputs() -> None:
    for output_root in [ROOT / "artifacts", RUN_ARTIFACT_ROOT]:
        if not output_root.exists():
            continue
        for path in sorted(output_root.glob("**/*.json")):
            data = load_json(path)
            label = artifact_label(path, output_root)
